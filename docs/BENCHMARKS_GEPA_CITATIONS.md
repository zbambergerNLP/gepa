...
| **3** | **HoVer** (many-hop fact extraction & claim verification, complete gold-doc retrieval, 3 documents) | **5** (FAPO, Feedback Descent, Reward-Free, PCO*, FlowBot*) | +5–10 pp typical; FAPO +33.8 pp | Three retrieval hops with two summarizers and two query generators. | Large (GEPA 52.33/51.67) |
...
|----------|-----------|-------------------------------------|-------------|-----------------------------------------------------|
| **1 — keep** | **HotpotQA** | #1 most beaten (6 papers, +3→+33 pp). FOREST Rev1 already has it; high headroom proves action-conditioning matters. | HF `hotpot_qa/fullwiki` (150/300/300, 6871 calls) | Two Wikipedia retrieval hops and four artifact components; also retain the 1-stage ablation. |
...
| **3 — keep** | **HoVer** | Tied #2 (5 papers), same shape as HotpotQA but three supporting documents — validates generality of HotpotQA findings. | Official HoVer v1.1, 150/300/300 (7051 calls) | Three live Wikipedia retrieval hops; score complete gold-page retrieval. |
...
## 5. Already Scaffolded vs TODO (FOREST `gepa` as of 2026-08-22)
...
| **HotpotQA** | ✅ `examples/hotpotqa/` | ✅ 150/300/300, 2 retrieval hops, 6871 calls | **Implemented** | Uses HF fullwiki, discards bundled context, retrieves through cached MediaWiki, and optimizes four artifact components. |
| **HoVer** | ✅ `examples/hover/` | ✅ 150/300/300, 3 retrieval hops, 7051 calls | **Implemented** | Uses official v1.1 three-document records, cached MediaWiki retrieval, and the artifact's complete-retrieval score. |
| **Terminal-Bench** | ✅ `examples/terminalbench/` | ✅ Harbor 0.22.0 + Terminal-Bench 3.0.0, immutable 74-task manifest | **Implemented** | Optimizes the Terminus system prompt, fixes tmux tools, disables skills/MCP for the first target, preserves full ATIF trajectories, and leaves turns uncapped within task timeouts. |
...
| **SWE-Bench Verified / Pro, AppWorld, SWE-Rebench** | ⚠️ `examples/appworld/` exists but not wired to GEPA beat | AppWorld exists stub; others missing | **TODO — Stretch** | No citation yet beats GEPA here — it would be novel. AppWorld 9.5% → still agentic. |
