...
COT_FORMAT_INSTRUCTION = (
    "\n\nDetermine the retrieval query, then write the requested value after a line containing exactly "
    f"'{FINAL_RESPONSE_MARKER}'. Only text after the marker is scored."
)
_SMOKE_RECORDS = (
    {
        "uid": "smoke-train",
...
def _call_lm(system: str, user: str, model: str, api_base: str | None) -> str:
    ...
    Returns:
        ...
        available. Returns an empty string when every context-window retry
        fails or the response contains neither field.
    """
    messages = []
    if system.strip():
...
def _supporting_titles(item: dict) -> list[str]:
    ...
    Returns:
        First-seen titles deduplicated by normalized form.
    """
...
def load_hover_dataset(
    ...
    Args:
        seed: Shuffle seed for the official release.
        data_dir: Directory containing or receiving the release file.
    ...
    Raises:
        ...
        ValueError: The release is not a JSON list or contains fewer than 750
            eligible three-document records.
    """
    if smoke:
...
def _render_passages(passages: Sequence[WikipediaPassage], max_chars: int = 12000) -> str:
    """Render a ranked passage prefix under the configured text total.
...
def run_two_stage(
    ...
    Returns:
        Serialized generated queries and all passages retrieved across the
        three hops.
    """
