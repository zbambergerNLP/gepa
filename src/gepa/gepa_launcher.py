...
def _template_consumer_model(adapter: Any, template_model: str | None) -> str | None:
    ...
    Args:
        adapter: Evaluation adapter that may expose a conventional model field.
        template_model: Explicit model override for template-family inference.

    Returns:
        Explicit or adapter-derived model identifier, or ``None`` when the
        adapter provides no recognized identifier.
    """
...
def optimize_anything(
    ...
    **Three optimization modes** (determined by ``dataset`` / ``valset``):
    ...
    Raises:
        TypeError: Candidate-selection or acceptance configuration has an
            unsupported type.
        ValueError: Required evaluators, objectives, reflection models,
            stopping conditions, cache storage, or prompt configuration are
            missing or mutually incompatible.
    ...
    # Preserve the model identifier for run-contract identity and the
