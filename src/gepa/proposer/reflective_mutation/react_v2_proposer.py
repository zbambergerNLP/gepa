...
Revise only the selected region of this structured {kind} document.
...
The template defines the allowed `## <Section>` headers and their order. Empty sections are absent from
the document until the harness gives them content. Edit body text only: never write, delete, rename, or
move a header line yourself.
...
def _lowered_semantic_tool(preferred_tool: EditTool | None, allowed_tools: Sequence[EditTool]) -> EditTool | None:
    """Return the hidden direct tool that the minimal set must reproduce."""
...
class ReActV2Proposer:
    ...
    def _apply_to_component(
        ...
        if edit_target.section is None:
            ...
            edited_component, executed = apply_edit(component_text, args)
            if self.template.rendered_sections(edited_component) != headers:
                raise MalformedDocumentError(
                    "Whole-document edits must preserve the existing '## <Section>' header sequence."
                )
            new_component = self.template.render(self.template.parse(edited_component))
            return new_component, new_component, executed
        region = self.template.parse(component_text)[edit_target.section]
        new_region, executed = apply_edit(region, args)
        new_component = self.template.replace_section_body(component_text, edit_target.section, new_region)
