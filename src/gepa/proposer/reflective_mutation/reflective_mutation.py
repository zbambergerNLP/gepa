...
class ReflectiveMutationProposer:
    ...
    def propose(self, state: GEPAState) -> list[CandidateProposal]:
        ...
        job_metadatas: list[Mapping[str, Any] | None] = [
            {
                ...
                "branch_edit_history": state.revision_history_for_candidate(p[0].parent_idx),
            }
            for p in prepared
            if p is not None
        ]
        ...
        for p in prepared:
            ...
            if not new_texts:
                # Reflection produced no text updates. A child would be
                # byte-identical to its parent: don't burn minibatch metric
                # calls evaluating it. But distinguish WHY it is empty.
                ...
                if dropped or attempt_records:
                    ...
                    # The action attempt genuinely happened even though no
                    # completed text survives to evaluate. Emit on_proposal_end (empty
                    # instructions) so the attempt counts toward the action's
                    # proposal total; skipping silently would inflate the
                    # action's apparent acceptance rate (#7).
                    ...
                    # Genuinely nothing to propose (e.g. every requested
                    # component was missing from the reflective dataset): no
                    # attempt to attribute, stay silent.
                    self.logger.log(
