...
"""Persistent guarded in-process Python environment of the RLM Proposer.
...
guarded executor, the delegation primitives, and the tree-wide budget accounting shared
...
The guarded executor restricts builtins, private attribute access, and imports
to plain data processing (no file, network, process or environment access) and
re-pins the context variables before every execution so rebinding ``component``
in Python never reaches the real document. It is a guardrail against a confused
or over-eager reflection LM. It runs in-process and is not a security boundary
against hostile code; only trusted reflection models may drive it.
"""
...
OS interfaces are not exposed. You have {turns} turns.
