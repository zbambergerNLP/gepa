...
RLM_ENVIRONMENT_CONTRACT: dict[str, Any] = {
...
class RLMBudget:
    ...
    def __post_init__(self) -> None:
        ...
        Raises:
            ValueError: An iteration or output limit is not positive, a shared
                counter or recursion depth is negative, or the optional
                execution timeout is not positive.
        """
        positive = {
...
def parse_action(reply: str, terminal: str) -> tuple[str, str]:
    ...
    Args:
        reply: The model's full answer-only reply.
...
class RLMEnvironment:
    ...
    def __init__(
        ...
        *,
        ...
        """Build one persistent guarded namespace for a trusted RLM.

        Context variables are pinned immediately, while non-context variables
        created by later executions persist for subsequent turns.

        Args:
            context: Read-only variables restored before every execution.
            lm: Model used for leaf queries and recursive child turns.
            budget: Limits shared across the complete recursion tree.
            usage: Existing tree-wide counters, or ``None`` for a new root.
            depth: Recursion depth of this environment.
        """
        ...
        if self.depth < self.budget.max_recursion_depth:
    ...
    def tools_help(self) -> str:
        ...
        Returns:
            ...
            while ``depth`` is below ``budget.max_recursion_depth``.
        """
        lines = [
    ...
    def llm_query(self, prompt: str) -> str:
        ...
        Returns:
            The sub-model's answer-only reply.
    ...
    def llm_query_batched(self, prompts: Sequence[str]) -> list[str]:
        ...
        with self._timer_paused():
            responses = [self.lm(prompt) for prompt in prompts]
    ...
    def _time_limit(self):
        ...
        Raises:
            RLMExecutionTimeout: A hard timeout cannot be installed for the
                current thread or the guarded body exceeds its time limit.
        """
        ...
        def _on_alarm(signum: int, frame: Any) -> None:
            """Turn the process alarm into a guarded-execution timeout.

            Args:
                signum: Signal number supplied by Python's signal dispatcher.
                frame: Interrupted frame supplied by the dispatcher.

            Raises:
                RLMExecutionTimeout: Always, to stop the guarded execution.
            """
