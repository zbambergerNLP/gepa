...
"""Persistent sandboxed Python environment of the RLM Proposer.

A Recursive Language Model (RLM) does not read its context in the prompt: the
context lives *outside* the model, as variables of a Python environment the
model drives one ``<python>`` block per turn. Variables the model assigns
persist from turn to turn, so it can decompose a large context (slice it,
search it with ``re``, count patterns in the traces) and keep intermediate state
without ever serializing that state back into its own prompt. Two delegation
primitives are bound into the environment: ``llm_query`` (a leaf LM call) and
``rlm_query`` (a child RLM with its own explicit context, its own persistent
environment and its own turns), plus batched variants of both.

:class:`RLMEnvironment` is that environment: it owns the namespace, the
sandbox, the delegation primitives, and the tree-wide budget accounting shared
between a root RLM and its children (:class:`RLMBudget`). It executes code and
records what happened (:class:`RLMExecution`, :class:`RLMCall`,
:class:`RLMStep`); it never commits anything. The proposer keeps authority over
state mutation: the environment's context variables are copies, and the only
way to change the candidate is the typed ``<edit>`` the model emits *outside*
Python (see :mod:`gepa.proposer.reflective_mutation.rlm_proposer`).

The sandbox restricts builtins and imports to plain data processing (no file,
network, process or environment access) and re-pins the context variables
before every execution so rebinding ``component`` in Python never reaches the
real document. It is a guardrail against a confused or over-eager reflection
LM, not a security boundary against hostile code.
"""
...
# Modules the sandbox may import (and pre-binds under their own names).
ALLOWED_MODULES: tuple[str, ...] = ("re", "json", "collections", "math", "statistics")

# Builtins the sandbox exposes. Everything with I/O, code loading or interpreter
# access (open, exec, eval, compile, input, globals, ...) is deliberately absent.
_SAFE_BUILTIN_NAMES: tuple[str, ...] = (
    "abs", "all", "any", "bool", "bytes", "callable", "chr", "dict", "dir", "divmod", "enumerate",
    "filter", "float", "format", "frozenset", "getattr", "hasattr", "hash", "int", "isinstance",
...
CHILD_RLM_PROMPT = """\
You are a sub-agent of an editing system. Answer ONE question about a piece of context.

## Question
{question}

## Your workspace
The context is NOT in this prompt. It is the read-only Python variable `context` (str, {context_chars} chars) \
...
{modules}, and
{tools}
There is no file, network or OS access. You have {turns} turns.
...
class RLMBudget:
    ...
    Args:
        ...
        max_exec_seconds: Wall-clock limit of one ``<python>`` execution,
            ...
            only on the main thread, where signals are available); ``None``
            disables it.
...
class RLMBudgetError(RuntimeError):
    ...
    ``<python>`` execution; sandboxed code sees it as an ordinary exception, so
...
class RLMCall:
    ...
    Attributes:
        ...
        response: The reply returned to the sandboxed code.
...
class _Usage:
    ...
    Batched primitives call the primitives from worker threads, hence the lock.
    """
...
def _format_error(exc: BaseException) -> str:
    """Render an exception raised by sandboxed code, keeping only the frames of that code.
    ...
    Returns:
        ...
        exception line, so the model sees its own code and not the sandbox
        internals.
    """
...
class RLMEnvironment:
    """The persistent sandboxed Python environment one RLM (root or child) drives.
    ...
    def __init__(
        ...
        *,
        ...
        """Build the sandbox namespace: safe builtins, allowed modules, delegation primitives, and the context."""
    ...
    def tools_help(self) -> str:
        ...
        lines = [
            ...
            "- llm_query_batched(prompts: list[str]) -> list[str]: the same for many independent questions at once.",
        ]
        if self.can_recurse:
            lines += [
    ...
    def llm_query_batched(self, prompts: Sequence[str]) -> list[str]:
        """Run independent ``llm_query`` calls concurrently, bound as ``llm_query_batched``.
        ...
        with self._timer_paused():
            responses = _run_concurrently([lambda p=p: strip_think_tags(self.lm(p)) for p in prompts])
    ...
    def rlm_query_batched(self, requests: Sequence[tuple[Any, str]]) -> list[str]:
        """Run independent ``rlm_query`` children concurrently, bound as ``rlm_query_batched``.
        ...
        with self._timer_paused():
            results = _run_concurrently([lambda c=c, p=p: self._run_child(c, p) for c, p in pairs])
    ...
    def _print(self, *args: Any, **kwargs: Any) -> None:
        """``print`` of the sandbox: writes to the per-execution capture buffer, never to the process stdout.
    ...
    def _restricted_import(
        ...
        """``__import__`` of the sandbox: only :data:`ALLOWED_MODULES` (already bound) may be imported.
        ...
        if level != 0 or name.split(".")[0] not in ALLOWED_MODULES:
            raise ImportError(
                f"import of {name!r} is not allowed in the RLM sandbox; available modules: {', '.join(ALLOWED_MODULES)}."
            )
        return builtins.__import__(name, globals, locals, fromlist, level)

    @contextmanager
    def _time_limit(self):
        """Bound one execution by ``max_exec_seconds`` (main thread only; elsewhere a no-op).
        ...
        ``SIGALRM``, or the caller is not the main thread (signals cannot be
        installed elsewhere).
...
def _as_text(context: Any) -> str:
    ...
    Args:
        context: Whatever the model passed to ``rlm_query`` (a string, or e.g. a
            list or dict it built in the sandbox).
...
def _run_concurrently(thunks: Sequence[Callable[[], Any]]) -> list[Any]:
    """Run independent thunks on a small thread pool and return their results in order.

    Args:
        thunks: Zero-argument callables; at most eight run at once.

    Returns:
        The thunks' return values in input order (an empty list for no thunks).
        An exception in any thunk propagates.
    """
    if not thunks:
        return []
