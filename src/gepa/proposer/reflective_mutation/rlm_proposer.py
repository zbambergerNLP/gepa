...
context, a persistent sandboxed Python environment, leaf LM delegation,
optional recursive RLM delegation, and deterministic edit commitment.
...
{tools}
There is no file, network or OS access. Rebinding `region` or `component` in Python has no effect on the real \
document (they are restored each turn): only your final <edit> changes it. You have {turns} turns in total.
...
class RLMProposer:
    """RLM Editor: reasons over externalized context in a sandboxed REPL, then commits one forced edit.
    ...
    def propose(
        ...
        section's body and the document re-rendered around it; a whole-document
        ...
        """
        bodies = self.template.parse(component_text)
        section = edit_target.section
        region_text = component_text if section is None else bodies[section]
        env = RLMEnvironment(
            {"region": region_text, "component": component_text, "feedback": feedback_summary, "traces": traces_text},
            self.lm,
            self.budget,
        )
        variables = "\n".join(
        ...
        for iteration in range(1, turns + 1):
            ...
            try:
                ...
                if section is None:
                    ...
                    new_component = self.template.render({**bodies, section: new_region})
            ...
            # max_chars=None means unbounded; the cap stops additive edits from
            # accreting text until the candidate exhausts the model context.
            if max_chars is not None and len(new_component) > max_chars:
                reason = f"over_budget ({len(new_component)} > {max_chars} chars)"
                return self._result(component_text, False, env, steps, iteration, final_output, dropped_reason=reason)
            return self._result(
                new_component, new_component != component_text, env, steps, iteration, final_output, executed_edit=ops
            )
        ...
        return self._result(component_text, False, env, steps, turns, final_output, dropped_reason=reason)
