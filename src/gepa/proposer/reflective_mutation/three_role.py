...
the Manifestor to steer ReAct V2 or the selected RLM ablation.
"""
...
from gepa.proposer.reflective_mutation.rlm_proposer import RLM_PROTOCOL_CONTRACT, RLMProposer
...
def _validated_chat_messages(messages: Sequence[Mapping[str, Any]]) -> list[dict[str, str]]:
    """Validate proposer chat messages before branch-history persistence.

    Args:
        messages: Role/content mappings produced by a proposer.

    Returns:
        Fresh dictionaries containing validated user or assistant messages.

    Raises:
        ValueError: A role is not user or assistant, or content is not a string.
    """
...
class ThreeRoleReflectionLM:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            rng: Seeded run RNG, or ``None`` for the deterministic default.
            logger: Optional run logger shared by all roles.
            ...
            proposer_backend: Primary ``react_v2`` or ``rlm`` ablation backend.
            ...
            rlm_budget: Optional tree-wide RLM budget override.

        Raises:
            ValueError: A level, tool set, backend, template family, component
                kind, or RLM/backend combination is invalid.
            TypeError: ``rlm_budget`` is neither :class:`RLMBudget` nor ``None``.
        """
    ...
    def run_contract(self, candidate: Mapping[str, str]) -> dict[str, Any]:
        ...
        return {
            ...
            "manifestor_delivery": ("user_message" if self.proposer_backend == "react_v2" else "rlm_prompt_guidance"),
            ...
            "max_proposer_model_calls": (
                ...
                if self.proposer_backend == "react_v2"
                    ...
                    self.rlm_budget.max_root_iterations
                    + self.rlm_budget.max_rlm_queries * self.rlm_budget.max_child_iterations
                    + self.rlm_budget.max_llm_queries
                )
            ),
            ...
            "rlm": (
                {
                    ...
                    "protocol": deepcopy(RLM_PROTOCOL_CONTRACT),
                }
    ...
    def _reflect_operated(
        ...
        Raises:
            ValueError: The level-2 Controller selects an RLM action without a
                coupled edit operator.
        """
        ...
        for name in components_to_update:
            ...
            manifested_text = steering_message or ""
            if self.proposer_backend == "react_v2":
