...
class DocumentTemplate:
    ...
    def edit_targets(self, component_name: str) -> list[EditTarget]:
        ...
        Targets come from the schema rather than the rendered text. An omitted
        empty section therefore remains available for a later edit.
    ...
    def rendered_sections(self, text: str) -> tuple[str, ...]:
        """Return the section headers currently present in a valid document.

        Args:
            text: Structured document to inspect.

        Returns:
            Present section names in rendered order.

        Raises:
            MalformedDocumentError: ``text`` does not conform to the template.
        """
        self.parse(text)
        return tuple(match.group(1) for match in _HEADER_RE.finditer(text))
    ...
    def render(self, bodies: dict[str, str]) -> str:
        ...
        the template and remain addressable through :meth:`edit_targets`.
