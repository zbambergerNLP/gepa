...
SEED_EVALUATION = ValsetEvaluation(
    outputs_by_val_id={0: "output-0.5"},
...
def accept(
    ...
    *,
    ...
    """Add one accepted candidate with a marker attempt record.

    Args:
        state: Optimizer state receiving the candidate.
        parents: Parent candidate indices.
        marker: Text identifying this branch revision.
        metadata: Explicit proposal metadata, or ``None`` to build a standard
            completed attempt.

    Returns:
        Index assigned to the accepted candidate.
    """
    ...
    return state.update_state_with_new_program(
        ...
        valset_evaluation=ValsetEvaluation(
            outputs_by_val_id={0: "output-0.6"},
...
def transcript(
    ...
    *,
    ...
    """Build the expected persistent transcript for one attempt.

    Args:
        marker: Text embedded in the attempted interaction.
        outcome: Accepted, rejected, or dropped optimizer result.
        score_before: Optional parent score.
        score_after: Optional proposal score.
        reason: Optional outcome explanation.

    Returns:
        Assistant attempt, tool observation, and optimizer feedback messages.
    """
    outcome_feedback = {
...
def test_seed_starts_with_no_global_revision_history() -> None:
    ...
    state = GEPAState({"sys": "seed"}, SEED_EVALUATION)
    assert deepcopy(state.revision_history_by_candidate[0]) == []
...
def test_descendant_inherits_only_its_parent_lineage() -> None:
    ...
    assert deepcopy(state.revision_history_by_candidate[child]) == transcript("parent")
    assert deepcopy(state.revision_history_by_candidate[grandchild]) == transcript("parent") + transcript("child")
    assert deepcopy(state.revision_history_by_candidate[0]) == []
...
def test_sibling_histories_are_isolated_and_never_become_global() -> None:
    ...
    assert deepcopy(state.revision_history_by_candidate[left]) == transcript("left-only")
    assert deepcopy(state.revision_history_by_candidate[right]) == transcript("right-only")
    assert "right-only" not in str(deepcopy(state.revision_history_by_candidate[left]))
    assert "left-only" not in str(deepcopy(state.revision_history_by_candidate[right]))


def test_result_snapshot_cannot_mutate_persistent_branch_history() -> None:
    """Detach the public result transcript from persistent optimizer state."""
    state = GEPAState({"sys": "seed"}, SEED_EVALUATION)
    child = accept(state, [0], "accepted")
    result = GEPAResult.from_state(state)
    assert result.revision_histories is not None
    exposed = result.revision_histories[child]
    exposed[0]["content"] = "mutated"
    exposed.append({"role": "user", "content": "injected"})
    assert deepcopy(state.revision_history_by_candidate[child]) == transcript("accepted")
...
def test_merge_inherits_only_the_exact_prefix_shared_by_all_parents() -> None:
    ...
    assert deepcopy(state.revision_history_by_candidate[merged]) == transcript("common") + transcript("merge")
...
def test_candidate_without_new_revision_records_inherits_parent_history_only() -> None:
    ...
    assert deepcopy(state.revision_history_by_candidate[merged]) == transcript("reflection")
...
def test_rejected_attempt_is_visible_only_when_reselecting_that_candidate() -> None:
    ...
    state.record_proposal_attempts(
        left,
        {
            "attempt_records": [
                {
                    "marker": "tried-and-regressed",
                    "chat_messages": [
                        {"role": "assistant", "content": "Assistant attempt: tried-and-regressed"},
                        {"role": "user", "content": "Tool observation: tried-and-regressed"},
                    ],
                }
            ]
        },
    ...
    assert deepcopy(state.revision_history_by_candidate[left]) == transcript("left") + transcript(
        "tried-and-regressed",
        "rejected",
        score_before=0.8,
        score_after=0.3,
    ...
    assert deepcopy(state.revision_history_by_candidate[right]) == transcript("right")
    assert deepcopy(state.revision_history_by_candidate[0]) == []
            ...
            "attempt_records": [
                {
                    "marker": "exhausted",
                    "chat_messages": [
                        {"role": "assistant", "content": "Assistant attempt: exhausted"},
                        {"role": "user", "content": "Tool observation: exhausted"},
                    ],
                }
            ]
        },
            ...
            "attempt_records": [
                {
                    "marker": "child",
                    "chat_messages": [
                        {"role": "assistant", "content": "Assistant attempt: child"},
                        {"role": "user", "content": "Tool observation: child"},
                    ],
                }
            ]
        },
    )
    history = deepcopy(state.revision_history_by_candidate[child])
    assert history == (
        transcript("parent")
        + transcript(
            "exhausted",
...
def test_proposal_time_snapshot_prevents_same_batch_sibling_leakage() -> None:
    ...
    state.record_proposal_attempts(
        0,
        {
            "attempt_records": [
                {
                    "marker": "dropped-sibling",
                    "chat_messages": [
                        {"role": "assistant", "content": "Assistant attempt: dropped-sibling"},
                        {"role": "user", "content": "Tool observation: dropped-sibling"},
                    ],
                }
            ]
        },
    ...
    child = accept(
        ...
        metadata={
            "attempt_records": [
                {
                    "marker": "accepted-sibling",
                    "chat_messages": [
                        {"role": "assistant", "content": "Assistant attempt: accepted-sibling"},
                        {"role": "user", "content": "Tool observation: accepted-sibling"},
                    ],
                }
            ],
    ...
    assert deepcopy(state.revision_history_by_candidate[0]) == transcript("dropped-sibling", "dropped")
    assert deepcopy(state.revision_history_by_candidate[child]) == transcript("accepted-sibling")
            ...
            "attempt_records": [
                {
                    "marker": "completed",
                    "chat_messages": [
                        {"role": "assistant", "content": "Assistant attempt: completed"},
                        {"role": "user", "content": "Tool observation: completed"},
                    ],
...
def test_revision_record_shape_is_validated_before_state_mutation(metadata: dict[str, Any]) -> None:
    ...
    Args:
        metadata: Invalid legacy revision-record shape under test.
    """
    state = GEPAState({"sys": "seed"}, SEED_EVALUATION)
...
def test_revision_history_survives_state_save_and_load(tmp_path) -> None:
    ...
    assert deepcopy(loaded.revision_history_by_candidate[child]) == transcript("accepted")
...
def test_attempt_outcome_records_the_exact_edit_target() -> None:
    """Keep section scope in user feedback without changing chat-message shape."""
    state = GEPAState({"sys": "seed"}, SEED_EVALUATION)
    child = accept(
        state,
        [0],
            ...
            "attempt_records": [
                {
                    "marker": "accepted",
                    "chat_messages": [
                        {"role": "assistant", "content": "Assistant attempt: accepted"},
                        {"role": "user", "content": "Tool observation: accepted"},
                    ],
                    "component": "sys",
                    "edit_target": "sys:Rules",
                }
            ]
        },
    )
    ...
    assert "Component: sys." in history[-1]["content"]
    assert "Edit target: sys:Rules." in history[-1]["content"]
    ...
    assert state_dict["revision_history_by_candidate"] == [
        [
            ...
            {"role": "user", "content": "replacement applied"},
            {
                "role": "user",
                "content": (
                    "Optimizer result: rejected; the branch remains unchanged. Score before: 0.8. "
                    "Score after: 0.2. Reason: did not help"
                ),
            },
        ]
    ]
