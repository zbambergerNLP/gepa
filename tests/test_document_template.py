...
class FakeMigrationLM:
    ...
    def __init__(self, *outputs: str) -> None:
        """Initialize the fake with replies to return in order.

        Args:
            outputs: Scripted migration replies, one per model call.
        """
        self.outputs = list(outputs)
        self.calls: list[str] = []

    def __call__(self, prompt: str) -> str:
        """Record one migration prompt and return the next scripted reply.

        Args:
            prompt: Migration prompt sent by the code under test.

        Returns:
            The next reply supplied at initialization.

        Raises:
            IndexError: No scripted replies remain.
        """
...
class TestEditTarget:
    ...
    def test_edit_target_label_names_its_component_and_section(self) -> None:
        """Test that an edit target exposes its component-scoped section label."""
        assert EditTarget("sys", "Rules").label == "sys:Rules"

    @pytest.mark.parametrize("section", [None, ""])
    def test_edit_target_rejects_missing_section(self, section) -> None:
        """Test that a whole-document or unnamed target cannot be represented.

        Args:
            section: Invalid section value passed to :class:`EditTarget`.
        """
