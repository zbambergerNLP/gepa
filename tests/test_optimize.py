...
def test_three_role_seed_must_be_in_canonical_format() -> None:
    ...
    with pytest.raises(MalformedDocumentError, match=r"'system_prompt'.*migrate_document"):
        optimize(
            ...
            trainset=mock_data,
            task_lm=task_lm,
            reflection_lm=Mock(return_value="noop"),
            reflection_level=1,
            max_metric_calls=2,
            reflection_minibatch_size=1,
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            Mock(return_value="noop"),  # reflection_lm
            2,  # reflection_level
            False,  # expected_derived
            id="callable_level_2",
        ),
...
def test_three_role_level2_derives_a_deterministic_manifestor_lm(
    ...
    Args:
        monkeypatch: Pytest fixture used to capture strategy construction.
    ...
    def stub_strategy(**kwargs):
        """Capture strategy arguments and stop before optimization begins.

        Args:
            **kwargs: Arguments supplied to ``ThreeRoleReflectionLM``.

        Raises:
            _StrategyCapturedError: Always, after capturing the arguments.
        """
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            None,  # template_family (omitted -> the 'auto' default)
            "openai/gpt-5",  # task_lm
            "openai",  # expected_family
            id="default_auto_maps_gpt_to_openai",
        ),
        pytest.param(
