...
def test_canonical_section_headers_are_protected_and_error_is_observable() -> None:
    """Reject a tool call that adds a new section header, then accept a safe retry."""
    lm = ScriptedLM(
        [
            tool_call(
                EditTool.INSERT_TEXT,
    ...
    assert "must have exactly the sections" in result.steps[0].observation
    assert "## Rogue" not in result.new_text
    assert "cite sources" in result.new_text
    TEMPLATE.parse(result.new_text)


def test_section_edit_preserves_every_byte_outside_the_selected_body() -> None:
    """Keep noncanonical-but-valid whitespace in all unselected regions unchanged."""
    component = PROMPT.replace(
        "## Role\nhelper\n\n## Task",
        " \t\n## Role   \n  helper  \n\n\n## Task",
    )
    TEMPLATE.parse(component)
    body_start, body_end = TEMPLATE.section_body_span(component, "Rules")
    prefix = component[:body_start]
    suffix = component[body_end:]
    lm = ScriptedLM([tool_call(EditTool.REPLACE_TEXT, target="be nice", text="be kind")])
    result = run(
        lm,
        preferred_tool=EditTool.REPLACE_TEXT,
    ...
    assert result.new_text.startswith(prefix)
    assert result.new_text.endswith(suffix)
    assert result.new_text == prefix + "- be kind\n- be brief" + suffix


def test_insert_into_empty_section_preserves_neighboring_section_bytes() -> None:
    """Insert into a zero-length body without rerendering adjacent sections."""
    component = TEMPLATE.render({"Rules": "- be brief"})
    role = EditTarget("sys", "Role")
    body_start, body_end = TEMPLATE.section_body_span(component, "Role")
    prefix = component[:body_start]
    suffix = component[body_end:]
    ...
    assert result.new_text == prefix + "helper" + suffix
    TEMPLATE.parse(result.new_text)
