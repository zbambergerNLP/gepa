...
"""Focused tests for bounded, guarded RLM execution."""
...
def environment(*, lm=None, budget: RLMBudget | None = None) -> RLMEnvironment:
    ...
    return RLMEnvironment(
        {"region": "be nice", "feedback": "too vague"},
        lm or EchoLM(),
        budget or RLMBudget(),
    )
...
def test_worker_thread_execution_fails_closed_when_signal_timeout_is_unavailable() -> None:
    ...
    worker = threading.Thread(target=lambda: outcomes.append(env.execute("while True:\n    pass")))
