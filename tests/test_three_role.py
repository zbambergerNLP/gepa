...
class ThreeRoleRLMLM(ThreeRoleLM):
    ...
    def __init__(self, rlm_replies: list[str]) -> None:
        """Store scripted RLM replies and initialize shared role logs.

        Args:
            rlm_replies: Root-editor replies consumed in order.
        """
        super().__init__([])
    ...
    def __call__(self, prompt: str | list[dict[str, Any]]) -> str:
        """Route root-editor prompts to RLM replies and delegate other roles.

        Args:
            prompt: String role prompt or ReAct message list.

        Returns:
            Scripted RLM reply or the base three-role response.

        Raises:
            AssertionError: Root-editor calls exceed scripted RLM replies.
        """
...
def test_rlm_backend_runs_controller_manifestor_and_retains_branch_chat() -> None:
    ...
    strat, _ = strategy(2, lm=lm, proposer_backend="rlm", rlm_budget=budget)
    history = [
        {"role": "assistant", "content": "role-sibling-marker"},
        {
            "role": "user",
    ...
    assert "role-sibling-marker" not in lm.rlm_calls[1]
    assert "helper" not in lm.rlm_calls[0]
    assert "helper" not in lm.rlm_calls[1]
